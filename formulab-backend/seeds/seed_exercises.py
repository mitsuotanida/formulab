...
EXERCISES = [
    ...
    },
    {
        ...
        "description": "BioFarm Chile planifica la producción de tres suplementos alimentarios durante tres meses, tomando simultáneamente decisiones de apertura de plantas, producción e inventario.\n\nÍndices y conjuntos:\n  I = {1,2}: plantas (i=1: PL1-Santiago, i=2: PL2-Antofagasta)\n  J = {P,V,C}: productos (Proteína, Vitaminas, Colágeno)\n  T = {1,2,3}: períodos mensuales (t=1: Enero, t=2: Febrero, t=3: Marzo)\n\nParámetros del modelo:\n  cᵢⱼ [$/u]: costo de producir el producto j en la planta i\n  rᵢⱼ [hr/u]: horas-máquina necesarias por unidad del producto j en la planta i\n  Capᵢ [hr/mes]: capacidad máxima de horas-máquina de la planta i por período (Cap₁=120, Cap₂=100)\n  fᵢ [$/mes]: costo fijo de operación de la planta i si opera ese período (f₁=200, f₂=150)\n  hⱼ [$/u/mes]: costo de mantener en inventario una unidad del producto j\n  dⱼₜ [u]: demanda del producto j en el período t (debe satisfacerse exactamente)\n  Iⱼ₀ = 0: inventario inicial de cada producto (cero para todo j)\n\nLos valores numéricos de cᵢⱼ, rᵢⱼ, hⱼ y dⱼₜ se encuentran en la tabla adjunta.\n\nDecisión de operación: si la planta i no opera en el período t, no puede producir ninguna unidad ese mes. Operar tiene un costo fijo fᵢ independiente del volumen producido.\n\nBalance de inventario: el inventario al inicio del período t (= Iⱼ,ₜ₋₁) más la producción de ese período debe cubrir exactamente la demanda dⱼₜ más el inventario que queda al final (= Iⱼₜ).\n\nSe pide: Formule el modelo de Programación Entera Mixta (MIP) que minimiza el costo total (producción + inventario + operación de plantas), definiendo explícitamente:\n  1. Conjuntos de índices (con su notación y descripción)\n  2. Parámetros (con su notación indexada y unidades)\n  3. Variables de decisión (continuas y binarias, con dominio y cuantificador ∀)\n  4. Función objetivo en notación de sumatoria (Σ) sobre los conjuntos definidos\n  5. Restricciones, indicando para cada una el conjunto de índices sobre el que aplica (∀i∈I, ∀j∈J, ∀t∈T)",
        "data_table": {
