...
def register(body: RegisterRequest, db: Session = Depends(get_db)):
    try:
        user = auth_service.register_user(db, body.email, body.name, body.password)
    ...
    return TokenResponse(
        ...
        user=UserBasic(id=str(user.id), name=user.name, email=user.email, role=user.role, xp=user.xp, level=user.level, streak=user.streak),
    )
